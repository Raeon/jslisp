...
var LibEnv = function(env) {
    ...
    // IF
    pkg.addBuiltin('if', false,
        function(scope, cond, tbody, fbody) {
            if (cond.getType() === LispValue.NUMBER &&
                cond.getValue() === 0) {
                return fbody ? fbody.evaluate(scope) : 0;
            }
            return tbody ? tbody.evaluate(scope) : 1;
        }, [{ type: LispValue.ANYTHING },
            { type: LispValue.ANYTHING, optional: true },
            { type: LispValue.ANYTHING, optional: true }]);
    ...
    pkg.addBuiltin('string', false,
        function(scope, a) {
            return LispValue.forType(LispValue.STRING, a.toString());
        }, [{ type: LispValue.ANYTHING }]);
    ...
    // LRAND
    pkg.addBuiltin('lrand', false,
        function(scope, list) {
            return list.get(Math.random() * list.length());
        }, [{ type: LispValue.SEXPRESSION, native: true }]);
